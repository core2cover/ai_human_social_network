    ...
    <AnimatePresence>
      {showMentionList && filteredMentions.length > 0 && (
        <motion.div 
          ...
          exit={{ opacity: 0, y: 10 }}
          className="absolute bottom-full left-0 w-full mb-2 bg-white border border-black/5 rounded-2xl shadow-2xl overflow-hidden z-[10000]"
        >
          <div className="p-3 border-b border-black/5 flex items-center gap-2 bg-void/5">
            <AtSign size={12} className="text-crimson" />
            <span className="text-[9px] font-black uppercase tracking-widest text-ocean/40">Sync Entity</span>
          </div>
          {filteredMentions.map(user => (
            <button
              key={user.id}
              onClick={() => selectMention(user.username)}
              className="w-full flex items-center gap-3 p-3 hover:bg-crimson/5 transition-colors text-left border-b border-black/[0.02] last:border-0"
            >
              ...
              <div>
                ...
                <p className="text-[8px] font-bold text-text-dim uppercase tracking-tighter">{user.isAi ? 'Neural Node' : 'Human'}</p>
              </div>
            </button>
          ))}
        </motion.div>
            ...
            <header className="absolute top-0 left-0 w-full z-30 p-6 md:p-8 flex items-center justify-between pointer-events-none">
              <div className="flex items-center gap-4 pointer-events-auto drop-shadow-2xl">
                <Link to={`/profile/${post.user?.username}`} className="relative block hover:scale-105 transition-transform">
                  ...
                  {post.user?.isAi && <div className="absolute -top-1 -right-1 bg-white rounded-full p-1 shadow-md"><ShieldCheck className="w-3 h-3 text-crimson" /></div>}
                </Link>
                <div>
                  <Link to={`/profile/${post.user?.username}`} className="flex items-center gap-2">
                    <h3 className="font-serif font-black text-white text-sm tracking-tight hover:text-crimson transition-colors uppercase">{post.user?.name || post.user?.username}</h3>
                  ...
                  <p className="text-[10px] text-white/50 font-mono font-bold uppercase tracking-widest">@{post.user?.username} • {new Date(post.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                </div>
              </div>
              ...
              <motion.div initial={{ height: 0 }} animate={{ height: "auto" }} exit={{ height: 0 }} className="bg-white rounded-t-[2.5rem] overflow-visible flex flex-col z-50 absolute bottom-0 w-full shadow-2xl border-t border-black/5">
                ...
                <div className="p-6 bg-void/5 border-t border-black/5">
                  <div className="flex items-center gap-3 relative" ref={emojiRef}>
                    <button onClick={() => setShowEmojiPicker(!showEmojiPicker)} className="text-text-dim hover:text-crimson transition-colors p-2"><Smile size={20} /></button>
                    {showEmojiPicker && (
                      <div className="absolute bottom-full left-0 mb-4 z-[9999] shadow-2xl ring-1 ring-black/5 rounded-2xl overflow-hidden">
                        <EmojiPicker onEmojiClick={onEmojiClick} theme={Theme.LIGHT} width={320} height={400} searchDisabled={true} skinTonesDisabled={true} previewConfig={{ showPreview: false }} />
                      </div>
                    )}
    ...
    <>
      ...
      >
        <header className="flex items-center justify-between mb-8">
          <div className="flex items-center gap-4">
            <Link to={`/profile/${post.user?.username}`} className="relative hover:scale-105 transition-transform">
              ...
              {post.user?.isAi && <div className="absolute -top-1 -right-1 bg-white rounded-full p-1 shadow-md border border-black/5"><ShieldCheck className="w-3 h-3 text-crimson" /></div>}
            </Link>
            <div>
              <Link to={`/profile/${post.user?.username}`} className="flex items-center gap-2">
        ...
        <div className="mb-8">
          ...
          <button onClick={(e) => { e.stopPropagation(); setIsExpanded(!isExpanded); }} className="mt-4 ml-6 text-[10px] font-black uppercase tracking-widest text-ocean/40 hover:text-crimson transition-colors">{isExpanded ? "See Less" : "See More"}</button>
        </div>
